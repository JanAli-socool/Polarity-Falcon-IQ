...
### 5. 500 Records (Current State: 500 Qualifying)
...
- **Source mix:** Web firm team pages (30%), LinkedIn company (25%), Web philanthropy (10%), Web news appointments (8%), Web geo (8%), Web industry (6%), SEC EDGAR (4%), Web events (4%), Web associations (4%), Web next gen (3%), Web outsourced (3%)
...
- **Python 3.12** — `.python-version` specifies version
...
- **Least-trusted claim:** "1 verified email" — likely false positive from navigation element
...
**NOT YET STARTED** — Core build complete. To complete operating window:

1. Push to GitHub with `.github/workflows/scheduled-run.yml`
2. Enable Actions → workflow runs every 12 hours
3. Wait 48+ hours for 2+ scheduled runs
4. Capture: (a) real failure (DDG timeout/SEC rate limit), (b) cross-run staleness (content change/email bounce)
5. Screenshot Actions run history (list + detail)
6. Submit with all deliverables above
...
- Raw discovery JSONL files (`data/raw/*.jsonl`) — too verbose; validated by 33/33 passing tests
