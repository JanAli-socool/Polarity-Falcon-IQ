# pipeline/07_build_final_dataset_v2.py
# Builds final canonical dataset from enriched people.
# Applies quality filters, deduplicates, assigns Record IDs, exports to canonical SQLite.
...
def confidence_for(row):
    ...
    if row["Contact Primary Email"]:
        score += 1
    if "team" in row["Contact Source URL"].lower() or "people" in row["Contact Source URL"].lower() or "about" in row["Contact Source URL"].lower():
        score += 1
...
# Select with diversity: cap per firm, prefer emails, team pages, high confidence
selected = []
for firm_name, rows in sorted(kept_by_firm.items()):
    ...
    selected.extend(rows[:10])  # up to 10 per firm
...
print("\n[info] Loading into canonical SQLite DB...")
init_db()
